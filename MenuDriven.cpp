...
bool MenuDriven:: Email_check(string email){
    const regex pattern("[K|k][0-9]{6}@(nu).(edu).(pk)");
    return regex_match(email,pattern);
}

bool MenuDriven:: Check_StudentID(string ID){
	const regex pattern("[K|k][0-9]{6}");
...
for(i=0; i<58; i++)
{
	...
	gotoxy(56,18);
	for(j=0;j<=30000000;j++);
	cout << "MARYIAM KHAN"<<endl;
	gotoxy(56,19);
	for(j=0;j<=30000000;j++);
	cout << "AIMAN NADEEM"<<endl;
	gotoxy(56,20);
	for(j=0;j<=30000000;j++);
	cout << "AMNA RAIS"<<endl;
